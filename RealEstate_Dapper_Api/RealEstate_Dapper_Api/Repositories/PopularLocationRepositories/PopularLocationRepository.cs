...
namespace RealEstate_Dapper_Api.Repositories.PopularLocationRepositories
{
    public class PopularLocationRepository : IPopularLocationRepository
	{
        ...
        public async Task DeletePopularLocation(int popularLocationID)
        {
            ...
            parameteres.Add("@locationID", popularLocationID);
            using (var connection = _context.CreateConnection())
            {
        ...
        public async Task<GetByIDPopularLocation> GetPopularLocation(int popularLocationID)
        {
            string query = "select * from PopularLocation Where LocationID=@popularLocationID";
            var parameters = new DynamicParameters();
            parameters.Add("@popularLocationID", popularLocationID);

            using (var connection = _context.CreateConnection())
            {
                var values = await connection.QueryFirstOrDefaultAsync<GetByIDPopularLocation>(query, parameters);
                return values;
            }
        }
